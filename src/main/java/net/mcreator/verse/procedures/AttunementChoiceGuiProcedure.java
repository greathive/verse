...
public class AttunementChoiceGuiProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			AttunementChoiceGuiProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				AttunementChoiceGuiProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event, double mouseX, double mouseY) {
		...
		if (Minecraft.getInstance().player != null) {
			...
			if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof AttunmentChoiceMenu) {
				...
				if (target(3)) {
					for (int index0 = 0; index0 < 9; index0++) {
						...
						mousedistance = Math.hypot(Math.abs(iconX - mouseX), Math.abs(iconY - mouseY));
						if (25 > mousedistance) {
							if (15 <= mousedistance) {
								scale = (1.4 + 7.5 / mousedistance) - mouseclick;
								angle = (7.5 / mousedistance) * 45.1 * (double) Math.sin(entity.tickCount / 10.1);
								order = -2;
							} else {
								scale = (1.4 + 0.5) - mouseclick;
						...
						if (entity.getData(VerseModVariables.PLAYER_VARIABLES).attunementchooserguivariable.contains("(" + texture + ")")) {
							render = true;
						} else if ((texture).equals("chain")) {
							render = true;
						} else if ((texture).equals("check")) {
							render = true;
						} else {
							render = false;
						}
