...
public class OutfitTableRenderProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			OutfitTableRenderProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				OutfitTableRenderProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (Minecraft.getInstance().player != null) {
			...
			if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof OutfitCraftingMenu) {
				renderEntity(world, entity, (Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2d), (Minecraft.getInstance().getWindow().getGuiScaledHeight() / 3d), (-50), 0, 0, 0, 60, false);
			}
		}
	}
}