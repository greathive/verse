...
public class RenderMantraSelectionProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RenderMantraSelectionProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				RenderMantraSelectionProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		double selectpos = 0;
		double rendernumber = 0;
		double renderXpos = 0;
		double renderYpos = 0;
		double mantracount = 0;
		double color = 0;
		double guiscaleoffset = 0;
		if (Minecraft.getInstance().player != null) {
			...
			if (entity.getPersistentData().getDouble("mantraselectiontick") != 0 && entity.tickCount - entity.getPersistentData().getDouble("mantraselectiontick") <= 100) {
				if (target(2)) {
					if (Minecraft.getInstance().getWindow().getGuiScale() == 1) {
						guiscaleoffset = 8;
					} else if (Minecraft.getInstance().getWindow().getGuiScale() == 2) {
						guiscaleoffset = -1;
					...
					anchory = Minecraft.getInstance().getWindow().getGuiScaledHeight() - Math.round(20 * (1 / Minecraft.getInstance().getWindow().getGuiScale())) + guiscaleoffset;
					selectedslot = entity.getPersistentData().getDouble("selected");
					selectpos = anchorx - 22 * 4.5 + selectedslot * 20 + 1;
					RenderSystem.setShaderTexture(0, ResourceLocation.parse(("verse" + ":textures/" + "screens/mantrahotbar" + ".png")));
					renderTexture((float) anchorx, (float) anchory, 0, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 4);
					RenderSystem.setShaderTexture(0, ResourceLocation.parse(("verse" + ":textures/" + "screens/selectedmantra" + ".png")));
					renderTexture((float) selectpos, (float) (anchory - 5), -1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 4);
					rendernumber = 1;
					mantracount = ReturnMantraCountProcedure.execute(entity, entity.getData(VerseModVariables.PLAYER_VARIABLES).mantrahotbar);
					for (int index0 = 0; index0 < 9; index0++) {
						renderXpos = anchorx - 22 * 4.5 + rendernumber * 20 + 3;
						if (rendernumber == selectedslot) {
							renderYpos = anchory - 5;
						} else {
							renderYpos = anchory - 4;
						}
						if (rendernumber <= mantracount) {
							color = 255;
						} else {
							color = 233;
						}
						renderTexts(("" + Math.round(rendernumber)), (float) renderXpos, (float) renderYpos, -2, -25, (float) 0.5, 255 << 24 | (int) color << 16 | (int) color << 8 | (int) color, 4);
