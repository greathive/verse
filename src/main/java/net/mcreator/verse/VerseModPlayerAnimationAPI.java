...
public class VerseModPlayerAnimationAPI {
	public static final Map<String, PlayerAnimation> animations = new Object2ObjectOpenHashMap<>();
	public static final Map<LivingEntity, PlayerAnimation> active_animations = new Object2ObjectOpenHashMap<>();
	...
	public static class PlayerBone {
		...
		public static Vec3 interpolate(List<Keyframe> keyframes, float time) {
			// For non-player entities (mobs), pass null as player
			return interpolate(keyframes, time, null);
		}
		...
		private static String preprocessMolangQueries(String expr, float time, LivingEntity entity) {
			...
			if (entity instanceof Player player) {
				return expr.replace("query.anim_time", fmt.apply(time))
				...
				// Handle generic LivingEntity queries (for mobs)
				return expr.replace("query.anim_time", fmt.apply(time))
						.replace("query.head_x_rotation", fmt.apply(Mth.wrapDegrees(entity.getXRot())))
						.replace("query.head_y_rotation", fmt.apply(Mth.wrapDegrees(entity.getYRot())))
						.replace("query.body_y_rotation", fmt.apply(Mth.wrapDegrees(entity.yBodyRot)))
						.replace("query.life_time", fmt.apply(entity.tickCount / 20.0f))
						.replace("query.health", fmt.apply(entity.getHealth()))
						.replace("query.max_health", fmt.apply(entity.getMaxHealth()))
						...
						.replace("query.hurt_time", fmt.apply((float) entity.hurtTime))
						.replace("query.speed", fmt.apply((float) entity.getDeltaMovement().length()));
			}

			// If no entity, just replace time
			return expr.replace("query.anim_time", fmt.apply(time));
		}
		...
		private static void loadAnimations(List<JsonObject> jsons, List<String> namespaces) {
			...
			for (JsonObject animationJson : jsons) {
				...
				if (sourceAnimations != null) {
					...
					VerseModPlayerAnimationAPI.loadAnimationFile(animationsWrapper);
				}
				namespaceIndex++;
			}
		}
