...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			VerseModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		if (animation == null) {
			...
			if (animation == null) {
				VerseMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			VerseModPlayerAnimationAPI.active_animations.put(player, animation);
		}

		// Get animation speed multiplier based on swing speed
		float speedMultiplier = SwingSpeedAnimationHelper.getAnimationSpeedMultiplier(player, playingAnimation);
